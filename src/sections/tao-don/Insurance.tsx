...
export default function Insurance({
  ...
  const FormSchema = z.object({
    ...
    noteOptions: z.record(z.string(), z.string().optional()).refine(
      (choices) => {
        if (!minhBachCanNang?.note_options) return true

        const noteOptions = minhBachCanNang.note_options
        const requiredKeys = noteOptions
          .filter(
            (noteOpt) =>
              noteOpt.list_note_options && noteOpt.list_note_options.length > 0,
          )
          .map((_, noteIdx) => `0-${noteIdx}`)
    ...
    noteOptionsAgreement: z.record(z.string(), z.boolean()).refine(
      (agreements) => {
        ...
        const noteOptions = minhBachCanNang.note_options
        const requiredKeys = noteOptions.map((_, noteIdx) => `0-${noteIdx}`)
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        {/* Render minh_bach_can_nang.note_options if available */}
        {minhBachCanNang?.note_options &&
          minhBachCanNang.note_options.map((noteOpt, noteIdx) => (
            <div
              key={noteIdx}
              className='rounded-[2.25rem] bg-white p-[1.5rem] shadow-[0_2px_6px_-1px_rgba(15,15,16,0.04)] xsm:rounded-[2rem] xsm:p-[1rem]'
            >
              ...
              {/* Render list_note_options as radio buttons */}
              {Array.isArray(noteOpt.list_note_options) &&
                noteOpt.list_note_options.length > 0 && (
                  <div className='space-y-[0.75rem] xsm:space-y-[0.5rem]'>
                    {noteOpt.list_note_options.map((opt, optIndex) => {
                      const fieldId = `noteOption-0-${noteIdx}-${optIndex}`
                      const radioFieldName = `noteOptions.0-${noteIdx}`
