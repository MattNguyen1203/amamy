...
const Reason = ({reasonsData}: IReason) => {
  ...
  //   })
  // }, [])
  if (!reasonsData.services) {
    return
  }
  return (
    <div className='flex w-full gap-[3.125rem] bg-[#F9FDFF] px-[6rem] py-[5rem] xsm:flex-col xsm:gap-[1.5rem] xsm:px-0 xsm:py-[2.5rem]'>
      {/* left */}
      <div className='flex h-[24.5625rem] w-[23.125rem] shrink-0 flex-col items-start justify-between xsm:h-full xsm:w-full xsm:px-[1rem]'>
      ...
      <div ref={containerGalleryRef}>
        <div
          ...
          className='hidden_scroll grid grid-cols-3 gap-x-[1.75rem] xsm:flex xsm:flex-row xsm:gap-x-[1rem] xsm:overflow-x-auto'
        >
          {Array.isArray(reasonsData?.services) &&
      ...
      <Link
        href={'tel:' + reasonsData?.phone?.replaceAll('.', '')}
        className='hidden w-fit items-center justify-center gap-[0.5rem] rounded-[1.75rem] bg-Blue-Primary py-[0.75rem] pl-[0.75rem] pr-[1.5rem] xsm:ml-[1rem] xsm:flex xsm:rounded-[1.375rem] xsm:py-[0.5rem] xsm:pl-[0.5rem] xsm:pr-[1.5rem]'
      >
