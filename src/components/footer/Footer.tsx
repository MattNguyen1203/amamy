  ...
  return (
    <div className='flex  px-24 pt-20 pb-[1rem] flex-col items-end gap-[3.4375rem] self-stretch bg-[#0F181D] text-white xsm:pt-[2rem] xsm:px-[1rem]'>
        <div className='flex w-full justify-between xsm:flex-col'>
          ...
          <div className='w-[37.4375rem] text-[var(--greyscaletext-0)] gap-[0.77rem] flex flex-col xsm:mt-[1.5rem] xsm:w-full'>
            ...
            <div className='relative w-full'>
                ...
                <i className='absolute right-3 top-1/2 -translate-y-1/2'><ArrowRightCircle /></i>
            </div>
          </div>
        </div>
        ...
        <div className='flex justify-between w-full mb-[6.25rem]'>
            <div className='flex gap-[1.5rem] flex-col'> 
                <h3 className='text-[1.25rem] not-italic font-bold leading-[120%]'>Liên hệ với chúng tôi</h3>
                <div className='flex flex-col gap-[1rem]'>
                <Address icon={LocationIcon()} content='Trụ sở 1: Schertlinstraße 17, 81379 München, Germany' />
                <Address icon={LocationIcon()} content='Trụ sở 2: Hanoi, VietNam' />
                <Address icon={LocationIcon()} content='Trụ sở 3: Ho Chi Minh, VietNam'/>
                <Address icon={LocationIcon()} content='Liên hệ hotline: 039.575.3374'/>
                <Address icon={LocationIcon()} content='Email: amamy.de@gmail.com'/>
                </div>
            </div>
            <div className='max-w-[35.875rem] flex-1 flex justify-between'>
                <div>
                    <h3 className='text-[1.25rem] mb-[1.5rem] not-italic font-bold leading-[120%]'>Dịch vụ</h3>
                    <div className='flex flex-col gap-[0.75rem]'>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Theo dõi bưu kiện</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Tính giá vận chuyển</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Quản trị hệ thống CNTT</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Hữu ích cho gửi hàng</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Dịch vụ khác</p>
                    </div>
                </div>
                <div>
                    <h3 className='text-[1.25rem] mb-[1.5rem] not-italic font-bold leading-[120%]'>Giải pháp</h3>
                    <div className='flex flex-col gap-[0.75rem]'>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Về Amamy</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Retail</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>ETO</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Edu</p>
                        <p className='text-[1rem] not-italic font-medium leading-[150%] opacity-80'>Media</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className='flex w-[88rem] p-6 justify-between relative items-center gap-2.5 stroke-[0.93px] opacity-20'>
            <div className='border-t-[1px] border-r border-l absolute top-0 right-0 left-0 rounded-t-[20px] h-3/4 w-full'></div>
            <p className='text-[#D6D6D6] text-[0.75rem] not-italic font-semibold leading-[normal] tracking-[-0.015rem] uppercase'>Tất cả quyền được bảo lưu. Bản quyền © 2024 Ltd.</p>
            <div className='flex gap-[2.2rem]'>
                <p className='text-[#D6D6D6] text-[0.75rem] not-italic font-semibold leading-[normal] tracking-[-0.015rem] uppercase'>Chính sách bảo mật</p>
                <p className='text-[#D6D6D6] text-[0.75rem] not-italic font-semibold leading-[normal] tracking-[-0.015rem] uppercase'>Điều khoản & Điều kiện</p>
            </div>
        </div>
    </div>
  );
...
const Address = ({icon, content}: {icon: any, content: string}) => {
    return <div className='flex gap-[0.5rem] items-center'>
            {icon}
        <p>{content}</p>
    </div>
}
...
const LocationIcon = () => {
    ...
    <path opacity="0.4" d="M17.1834 7.04183C16.3084 3.19183 12.9501 1.4585 10.0001 1.4585C10.0001 1.4585 10.0001 1.4585 9.99175 1.4585C7.05008 1.4585 3.68341 3.1835 2.80841 7.0335C1.83341 11.3335 4.46675 14.9752 6.85008 17.2668C7.73341 18.1168 8.86675 18.5418 10.0001 18.5418C11.1334 18.5418 12.2667 18.1168 13.1417 17.2668C15.5251 14.9752 18.1584 11.3418 17.1834 7.04183Z" fill="white"/>
...
const IconWrap = (icon:any) => {
    return        <div className='flex w-[3rem] h-[3rem] p-3 flex-col items-center gap-[-2.5rem] rounded-[var(--12)] border-[1px] border-solid
